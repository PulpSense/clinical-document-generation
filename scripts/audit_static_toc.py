...
def expected_toc_entries(docx_path: Path) -> list[str]:
    """Derive the required static index from final Word constructs.

    The reference template's static rows are not a reliable source of truth:
    they can omit a section or a data-driven table.  The final document's
    heading and caption constructs are the authoritative expected set.
    """
    with zipfile.ZipFile(docx_path) as archive:
        document = ET.fromstring(archive.read("word/document.xml"))
    expected: list[str] = []
    for paragraph in document.iter(W + "p"):
        ...
        is_heading = style is not None and style.get(W + "val", "").lower().startswith("heading")
        if is_heading and text and text not in expected:
            expected.append(text)
        elif text.startswith("Table ") and not re.search(r"\.{3,}\s*\d+\s*$", text) and paragraph.find(f".//{W}tab") is None and text not in expected:
            expected.append(text)
    return expected
...
def page_contains_heading(title: str, text: str, allow_substring: bool = False) -> bool:
    ...
    def heading_key(value: str) -> str:
        """Compare headings across renderer punctuation and case differences."""
        return re.sub(r"[^a-z0-9]+", "", normalize(value).casefold())

    title_key = heading_key(title_norm)
    if any(heading_key(line) == title_key for line in lines):
        return True
    ...
    if not allow_substring:
        return False
    return title_norm.casefold() in page_norm.casefold() or title_key in heading_key(page_norm)
...
def audit(docx_path: Path, pdf_path: Path) -> dict[str, Any]:
    ...
    expected = expected_toc_entries(docx_path)
    listed_keys = {_title_key(entry["title"]) for entry in entries}
    missing_entries = [title for title in expected if _title_key(title) not in listed_keys]
    ...
    missing.extend(
        {
            ...
            "listed_page": None,
            "actual_page": None,
            "searched_after_toc": True,
            "ok": False,
            "reason": "Required heading or table caption is absent from the Static TOC.",
        }
        for title in missing_entries
    )
    return {
        "docx": str(docx_path),
        "pdf": str(pdf_path),

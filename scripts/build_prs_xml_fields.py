...
PRS_TABLE_COLUMNS = {
    "interventions": [
        ...
        {"key": "interventionName", "label": "Intervention Name"},
        {"key": "interventionDescription", "label": "Intervention Description"},
        {"key": "armGroupLabel", "label": "Arm Group Label"},
    ],
    "arm_groups": [
        {"key": "armGroupLabel", "label": "Arm Group Label"},
        {"key": "armGroupType", "label": "Arm Group Type"},
        {"key": "armGroupDescription", "label": "Arm Group Description"},
    ],
    "outcomes": [
        {"key": "outcomeMeasure", "label": "Outcome Measure"},
        {"key": "outcomeTimeFrame", "label": "Outcome Time Frame"},
        {"key": "uid", "label": "UID"},
...
def infer_masking(reference: dict) -> str:
    ...
    if explicit:
        if str(explicit).strip().casefold() in {"none", "not masked", "open label", "open-label"}:
            return ""
        return explicit
...
def structured_table(columns: list[dict], rows: list[dict]) -> dict:
    return {"columns": [dict(column) for column in columns], "rows": rows}


def outcome_table_rows(items: list[dict]) -> list[dict]:
    return [
        {
            "outcomeMeasure": item.get("measure", ""),
            "outcomeTimeFrame": item.get("time_frame", ""),
            "uid": item.get("uid", ""),
...
def prs_xml_tables(
    ...
    return {
        "interventions": structured_table(
            PRS_TABLE_COLUMNS["interventions"],
            [
                {
                    "interventionType": item.get("type", ""),
                    "interventionName": item.get("name", ""),
                    "interventionDescription": item.get("description", ""),
                    "armGroupLabel": item.get("arm_group_label", ""),
                }
                for item in intervention_items
            ],
        ),
        "arm_groups": structured_table(
            PRS_TABLE_COLUMNS["arm_groups"],
            [
                {
                    "armGroupLabel": item.get("label", ""),
                    "armGroupType": item.get("type", ""),
                    "armGroupDescription": item.get("description", ""),
                }
                for item in arm_items
            ],
        ),
        "primary_outcomes": structured_table(PRS_TABLE_COLUMNS["outcomes"], outcome_table_rows(primary_items)),
        "secondary_outcomes": structured_table(PRS_TABLE_COLUMNS["outcomes"], outcome_table_rows(secondary_items)),
        "other_outcomes": structured_table(PRS_TABLE_COLUMNS["outcomes"], outcome_table_rows(other_items)),
    }
...
def build_fields(reference: dict, template_path: Path) -> tuple[dict, list[dict]]:
    ...
    existing_tables = existing.get("data_driven_tables") if isinstance(existing.get("data_driven_tables"), dict) else {}
    data_driven_tables = dict(existing_tables)
    data_driven_tables["prs_xml"] = prs_xml_tables(
        intervention_items,
        arm_items,
        primary_items,
        secondary_items,
    ...
    fields["data_driven_tables"] = data_driven_tables

...
CONTENT_COMPLETENESS_CHECKLIST_VERSION = "2026-08-18"
PROTOCOL_DOC = ("protocol_docx", "Generated Protocol")
ICF_DOC = ("icf_docx", "Generated ICF")
XML_DOC = ("xml", "Generated PRS XML")
...
def _content_item(
    ...
    return field, label, artifact, document_key, document_type


PROSPECTIVE_CONTENT_CHECKS = [
    _content_item("template_fields.AI_populationShort", "Protocol population summary", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_introduction", "Protocol introduction", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_populationLong", "Protocol population section", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_inclusionCriteria", "Protocol inclusion criteria", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_exclusionCriteria", "Protocol exclusion criteria", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_studyDesignLong", "Protocol study design section", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_methods", "Protocol methods section", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_visitSchedule", "Protocol visit schedule overview", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_visitScheduleDetails", "Protocol visit schedule details", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_measurements", "Protocol measurements overview", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_measurementsDetails", "Protocol measurement details", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_analysisDataSets", "Protocol analysis dataset section", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_statisticalMethodology", "Protocol statistical methodology", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_statisticalConsiderations", "Protocol statistical considerations", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_risks", "Protocol risk section", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item(
        "template_fields.data_driven_tables.visit_schedule.rows",
        "Visit schedule table rows",
        "Data-Driven Table",
        *PROTOCOL_DOC,
    ),
    _content_item("template_fields.AI_studyPurpose", "ICF study purpose", "Study-Specific Body", *ICF_DOC),
    _content_item("template_fields.AI_icfVisitsOverview", "ICF visit overview", "Study-Specific Body", *ICF_DOC),
    _content_item("template_fields.AI_visitsDetails", "ICF visit details", "Study-Specific Body", *ICF_DOC),
    _content_item("template_fields.AI_visitsAndLength", "ICF visit length and participants", "Study-Specific Body", *ICF_DOC),
    _content_item(
        "template_fields.AI_interventionPossibleSideEffects",
        "ICF intervention side effects",
        "Study-Specific Body",
        *ICF_DOC,
    ),
    _content_item(
        "template_fields.data_driven_tables.prs_xml.primary_outcomes.rows",
        "PRS primary outcome rows",
        "Data-Driven Table",
        *XML_DOC,
    ),
]

RETROSPECTIVE_CONTENT_CHECKS = [
    _content_item("template_fields.AI_introduction", "Protocol introduction", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_objectivesIntro", "Protocol objectives introduction", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_primaryOutcome", "Protocol primary outcome section", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_populationLong", "Protocol population section", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_inclusionCriteria", "Protocol inclusion criteria", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_exclusionCriteria", "Protocol exclusion criteria", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_studyDesignLong", "Protocol study design section", "Study-Specific Body", *PROTOCOL_DOC),
    ...
    _content_item("template_fields.AI_analysisDataSets", "Protocol analysis dataset section", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item(
        "template_fields.AI_analysisDataSetsBullets",
        "Protocol analysis dataset bullets",
        "Study-Specific Body",
        *PROTOCOL_DOC,
    ),
    _content_item("template_fields.AI_statisticalMethodology", "Protocol statistical methodology", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_statisticalConsiderations", "Protocol statistical considerations", "Study-Specific Body", *PROTOCOL_DOC),
    _content_item("template_fields.AI_studyProcedure", "Protocol study procedure section", "Study-Specific Body", *PROTOCOL_DOC),
]


def _content_checks(items: list[tuple[str, ...]]) -> list[dict[str, str]]:
    checks = []
    for item in items:
        field, label, artifact = item[:3]
        document_key = item[3] if len(item) > 3 else "protocol_docx"
        document_type = item[4] if len(item) > 4 else "Generated Protocol"
        checks.append(
            {
                "field": field,
                "label": label,
                ...
                "checklist_version": CONTENT_COMPLETENESS_CHECKLIST_VERSION,
            }
        )
    return checks
...
BRANCHES = {
    "prospective": {
        ...
        "content_completeness": _content_checks(PROSPECTIVE_CONTENT_CHECKS),
        "required_paths": [
            "generated.protocol",
            "generated.icf",
    ...
    "retrospective": {
        ...
        "content_completeness": _content_checks(RETROSPECTIVE_CONTENT_CHECKS),
        "required_paths": [
            "generated.protocol",
        ],
    },
...
def content_completeness_missing(reference: dict) -> list[dict]:
    meta = reference.get("meta") if isinstance(reference.get("meta"), dict) else {}
    branch = branch_for_study_type(meta.get("study_type"))
    if not branch:
        return []
    raw_document_set = meta.get("document_set")
    document_set = set(raw_document_set) if isinstance(raw_document_set, list) else set()
    canonical = branch["canonical_study_type"]
    missing = []
    for check in branch.get("content_completeness", []):
        document_key = check.get("document_key")
        if document_set and document_key and document_key not in document_set:
            continue
        field = check["field"]
        if is_meaningful_value(get_path(reference, field)):
            continue
        document_type = check.get("document_type") or "Generated Protocol"
        missing.append(
            {
                "field": field,
                "issue": (
                    f"{check['gate']} requires {check['label']} "
                    f"before the {document_type} is ready for delivery."
                ),
                "gate": check["gate"],
                "artifact": check["artifact"],
                "document_key": document_key,
                "document_type": document_type,
                "checklist_version": check["checklist_version"],
...
def repair_report_markdown(missing: list[dict]) -> str:
    lines = [
        "# Repair Report",
        "",
        "The generated document set is not ready for delivery because one or more Delivery Gates failed.",
        "",
    ]
    if not missing:
        lines.append("No Delivery Gate failures were detected.")
        return "\n".join(lines) + "\n"

    grouped: dict[str, list[dict]] = {}
    for item in missing:
        grouped.setdefault(str(item.get("gate") or "Delivery Gate"), []).append(item)
    ...
    for gate, items in grouped.items():
        ...
        for item in items:
            ...
            version = item.get("checklist_version")
            details = [str(artifact)] if artifact else []
            if document_type:
                details.append(str(document_type))
            if version:
                details.append(f"checklist {version}")
            suffix = f" ({', '.join(details)})" if details else ""
            lines.append(f"- `{field}`{suffix}: {item.get('issue', 'Review required.')}")
        lines.append("")

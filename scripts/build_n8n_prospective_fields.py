...
def visit_rows_with_report(reference: dict) -> tuple[list[dict], dict | None]:
    ...
    schedule_candidates = [
        ("generated.protocol.visitSchedule", protocol.get("visitSchedule")),
        ("procedures.visit_schedule", get_path(reference, "procedures.visit_schedule")),
        ("study.timeline", get_path(reference, "study.timeline")),
    ]
    visit_schedule = ""
    visit_schedule_source = ""
    for source, value in schedule_candidates:
        visit_schedule = first_text(value)
        if visit_schedule:
            visit_schedule_source = source
            break
    if visit_schedule:
        parsed_rows = []
        lines = [line.strip() for line in re.split(r"\r?\n|;", visit_schedule) if line.strip()]
            ...
            parsed_rows.append(
                {
                    ...
                    "visitName": parts[0],
                    "visitWindow": parts[1],
                    "CRFnumber": parts[2] if len(parts) > 2 else "",
                }
            )
        if parsed_rows:
            return parsed_rows, {
                "source": visit_schedule_source,
                "issue": "Legacy string fallback converted an unambiguous delimited schedule into structured rows.",
            }
        return [], {
            "source": visit_schedule_source,
            "issue": "Legacy schedule string is ambiguous; provide structured visit rows with visit names, windows, and CRF values.",
            "blocking": True,
        }
    return [], None
...
def visit_rows(reference: dict) -> list[dict]:
    rows, _ = visit_rows_with_report(reference)
    return rows


def visit_schedule_table(reference: dict) -> dict:
    rows, fallback = visit_rows_with_report(reference)
    table = {
        "columns": [dict(column) for column in VISIT_TABLE_COLUMNS],
        "rows": rows,
    }
    if fallback:
        table["legacy_string_fallback"] = fallback
    return table
...
def build_fields(reference: dict) -> dict:
    ...
    visit_schedule = visit_schedule_table(reference)
    visit_table = visit_schedule["rows"]
    existing_tables = get_path(reference, "template_fields.data_driven_tables")
    data_driven_tables = dict(existing_tables) if isinstance(existing_tables, dict) else {}
    data_driven_tables["visit_schedule"] = visit_schedule
    ...
    fields = {
        "visitsTable": "",
        "data_driven_tables": data_driven_tables,

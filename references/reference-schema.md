...
- `template_fields`: compatibility values for client templates that use legacy flat placeholders, especially n8n-style protocol fields such as `AI_shortTitle` and `protocolNumber`. Also stores `data_driven_tables` for structured DOCX/XML repeated rows used by rebuilt templates.
...
For prospective or ambispective PRS XML, run `scripts/build_prs_xml_fields.py` after the n8n mapper. It populates PRS placeholders under `template_fields`, sets `regulatory.xml_profile` to `clinicaltrials-prs`, records repeated XML counts under `template_fields.__prs_counts`, and exposes structured repeated XML rows under `template_fields.data_driven_tables.prs_xml`.

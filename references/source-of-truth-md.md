...
5. Run `scripts/check_required_inputs.py --run-dir <run-dir>` again.
6. If the upload message clearly approved final generation, rerun parse with `--approval-status approved --approved-by "<reviewer>"` or set approval afterward with `scripts/set_approval.py`.
...
- `build_n8n_ambispective_fields.py`

...
Before the source-of-truth step, confirm `meta.icf_template` for every prospective or ambispective run. If the input names neither supported template, ask which to use. If it names another IRB, explain that only Advarra and Sterling are available and ask which one to use. Pass the choice to the public workflow's `prepare` stage; it applies the selected template internally.
...
2. Draft the internal reference from the source material, then run the public workflow's `prepare` stage. It owns the missing-input preflight.

If `reference/missing-inputs.md` lists blocking inputs, ask the reviewer for them before creating the structured source Markdown. Clinical blockers are missing or conflicting starred Fillout fields; Prospective/Ambispective runs also require the separate ICF template choice.

3. The public workflow's `prepare` stage creates the reviewer-facing Markdown source document when the preflight passes.
...
4. After approval, the public workflow selects the branch adapter and populates the legacy n8n and PRS XML placeholders internally. It blocks on required gaps and retains nonblocking diagnostics as internal evidence.

5. Record approval through the public workflow's `approve` stage when the reviewer approves the source Markdown, unless approval was already recorded while parsing an uploaded file.

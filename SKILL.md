...
- Normal generation is not software maintenance. During ordinary client generation, the installed skill and its templates, contracts, tests, and implementation remain read-only; only the run workspace and isolated runtime caches may be written. When the owner explicitly requests development maintenance, Hermes may edit the Git-managed development checkout and push those changes through the authorized Git/GitHub workflow. Development edits must be tested, committed, repackaged, and re-certified before they can affect an installed or active release; Hermes must never edit the active certified release in place.

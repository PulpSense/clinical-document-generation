...
Future Hermes clinical-document runs target 10–20 minutes and have one persisted 45-minute ceiling. Environment and tooling failures must not prevent construction of a complete branch candidate, but client delivery remains atomic and requires mandatory Visual QA.
...
- Keep all rendering and review local. A delegated visual-review timeout routes the same request to the parent reviewer. The parent callback completes only when every exact response path passes the existing validator and request/revision binding checks. One unchanged-request parent retry is permitted; a second absent or invalid response blocks immediately as `missing_parent_visual_review_response` instead of looping to the operation deadline.
...
- Permit an explicitly requested `manual_pre_release` operation from a directly extracted, manifest-valid unsigned candidate only after its bundled PDFium runtime is provisioned and its host-office/PDFium path passes installation smoke. Record the mode and candidate fingerprint; this exception does not confer promotion or certification, and it cannot enter the certified installation path.
...
- At the 45-minute ceiling, retain a complete unresolved candidate internally and publish no client outputs.
...
- Bind operation identity to the Promoted Release fingerprint, or to the manifest fingerprint for an explicit `manual_pre_release` operation, plus compatible runtime history, exact pending handoffs, attempt counters, stage timings, soft-budget diagnostics, cleanup evidence, and terminal result. A missing worker response may redispatch only the unchanged request.
- Treat stage budgets as soft early-fallback or diagnostic thresholds. Only the original 45-minute UTC deadline is terminal, and late worker or opener completion cannot publish or confirm after it.

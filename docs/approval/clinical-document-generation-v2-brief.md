...
-> allow up to three total attempts
-> if it still fails, release nothing and produce a Repair Report
...
The skill will reuse sections that already passed and retry only the failed parts, up to three total attempts. The full branch package will be released together; partial document packages will never be delivered.
